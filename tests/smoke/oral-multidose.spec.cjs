...
// --- scope disclaimer --------------------------------------------------------
// The old .disclaimer-box was already above the inputs, which was right, but it
// was not collapsible and failed AA in both themes. The card keeps the position
// and names what the model cannot represent, active metabolites above all.

test('oral-multidose scope disclaimer sits above the tool and reads while collapsed', async ({ page }) => {
  await page.goto('/tools/oral-multidose.html', { waitUntil: 'domcontentloaded' });
  ...
  await expect(summary).toContainText('Not dosing guidance');
...
test('oral-multidose disclaimer names its specific omissions', async ({ page }) => {
  await page.goto('/tools/oral-multidose.html', { waitUntil: 'domcontentloaded' });
...
test('oral-multidose disclaimer carries the five legal elements', async ({ page }) => {
  await page.goto('/tools/oral-multidose.html', { waitUntil: 'domcontentloaded' });
  await page.locator('#scopeDisclaimer').evaluate((el) => { el.open = true; });
...
test('oral-multidose carries a second touchpoint beside the output', async ({ page }) => {
  await page.goto('/tools/oral-multidose.html', { waitUntil: 'domcontentloaded' });
  const touchpoint = page.locator('p.disclaimer');
  await expect(touchpoint).toHaveCount(1);
  await expect(touchpoint).toContainText('A model, not a person');
  await expect(touchpoint).toBeVisible();
});

test('oral-multidose disclaimer text clears WCAG AA in both themes', async ({ page }) => {
  await page.goto('/tools/oral-multidose.html', { waitUntil: 'domcontentloaded' });
...
test('oral-multidose disclaimer keeps the clauses the old block carried', async ({ page }) => {
  await page.goto('/tools/oral-multidose.html', { waitUntil: 'domcontentloaded' });
  await page.locator('#scopeDisclaimer').evaluate((el) => { el.open = true; });
  const footer = page.locator('#scopeDisclaimer .disclaimer-footer');
